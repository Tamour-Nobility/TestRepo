...
@Component({
    selector: 'charges-by-primary-dx-dos-report',
    templateUrl: './charges-by-primary-dx-dos.component.html'
})
export class ChargesByPrimaryDxDOS implements OnInit {
    form: FormGroup;
    ...
    onSearch() {
        if (this.canSearch()) {
            ...
            this.API.PostData('/Report/ByPrimaryDXDos', this.request, (res) => {
...
//        this.mappedReportData = data.map(obj => Object.values(obj).map((v, index) => index > 1 ? this.currency.transform(v) : v));
        ...
        this.dataTable = table.DataTable({
            ...
            buttons: this.datatableService.getExportButtons(['Charges by Primary DX', this.request.dateFrom, this.request.dateTo]),
        });
    }
