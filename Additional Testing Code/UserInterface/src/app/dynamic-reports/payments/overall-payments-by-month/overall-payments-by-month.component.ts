...
@Component({
    selector: 'overall-payments-by-month-report',
    templateUrl: './overall-payments-by-month.component.html'
})
export class OverallPaymentsByMonth implements OnInit {
    data: any;
    form: FormGroup;
    ...
    onSearch() {
        if (this.canSearch()) {
            ...
            this.API.PostData('/Report/PaymentMonthWise', this.request, (res) => {
    ...
    updateDatatable(data) {
        ...
        this.chRef.detectChanges();
        const table: any = $('.datatablePaymentMonthly');
        this.dataTable = table.DataTable({
            ...
            buttons: this.datatableService.getExportButtons(['Overall Payments Monthly', this.request.dateFrom, this.request.dateTo]),
            order: [0, 'desc']
        });
    }
    ...
    updateChart(data) {
        ...
        this.barChartData = [{ data: data.map(d => d.PAYMENT), label: 'Monthly Payments' }];
    }
