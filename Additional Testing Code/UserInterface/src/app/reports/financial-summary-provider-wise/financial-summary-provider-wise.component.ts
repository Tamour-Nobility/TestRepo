...
import { FinancialAnalysisProvider, PracticesList } from '../classes/aging-summary-report-model';
import { APIService } from '../../components/services/api.service';
import { Common } from '../../services/common/common';
import { DatatableService } from '../../../app/services/data/datatable.service';

@Component({
  selector: 'app-financial-summary-provider-wise',
  templateUrl: './financial-summary-provider-wise.component.html'
})
export class FinancialSummaryProviderWiseComponent implements OnInit {
  FSPWForm: FormGroup;
  strToDate: string = "";
  strFromDate: string = "";
  dataTable: any;
  listFinancialSummary: FinancialAnalysisProvider[];
  ...
  getAgingSummaryReport() {
    if (!Common.isNullOrEmpty(this.strFromDate) && !Common.isNullOrEmpty(this.strToDate) && !Common.isNullOrEmpty(this.ddlPracticeCode)) {
      ...
      this.API.getData('/Report/FinancialAnalysisByProviderandProcCodesReport?PracticeCode=' + this.ddlPracticeCode + '&DateFrom=' + this.strFromDate + '&DateTo=' + this.strToDate).subscribe(
        d => {
          if (d.Status == "Sucess") {
            if (this.dataTable) {
            ...
            this.dataTable = table.DataTable({
              ...
              buttons: this.datatableService.getExportButtons(['Provider wise Financial Summary', this.strFromDate, this.strToDate]),
            });
          }
          else {
